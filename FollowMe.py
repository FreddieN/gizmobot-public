...
#Prototyping code for what was developed as 'follow me' mode in the app
...
async def connect(device):
    async with BleakClient(device) as client:
        ...
        command = f"speed075".encode()  #lower speed to avoid injury
        ...
        while True:
            ...
            dir_matrix = ['back', 'rotatecw', 'rotateacw', 'forward']
            direction = sensor_vals.index(min(sensor_vals))
            if(sensor_vals[direction] < 200 ): #check which sensor is covered
                print(dir_matrix[direction])
                command = dir_matrix[direction].encode()  #head in that direction
                await client.write_gatt_char("6E400002-B5A3-F393-E0A9-E50E24DCCA9E", command, response=True)
                print("Command sent successfully.")
            sleep(1)
            command = "stop".encode()  
